"""
Best Buy Spider for Unified E-commerce Product Data Aggregator
"""
import scrapy
from scrapy import Request
from ..items import BestBuyProductItem, ImageItem, SpecificationItem, VariationItem, ReviewItem
...
class BestBuySpider(BaseSpider):
    name = 'bestbuy'
    allowed_domains = ['bestbuy.com', 'bestbuy.ca']
    start_urls = [
        'https://www.bestbuy.com/site/electronics/abcat0100000.c',
        'https://www.bestbuy.com/site/computers-tablets/abcat0500000.c',
        'https://www.bestbuy.com/site/home-appliances/abcat0910000.c',
        'https://www.bestbuy.com/site/health-fitness-beauty/abcat1000000.c',
        'https://www.bestbuy.com/site/toys-games/abcat0700000.c',
    ]
    
    def start_requests(self):
        """Start requests for Best Buy categories"""
        for url in self.start_urls:
            yield self.make_request(url=url, callback=self.parse_category_page)

    def parse_category_page(self, response):
        """
        Parse Best Buy category listing pages and extract product links
        """
        ...
        if not product_links:
            product_links = response.css('div[data-testid="product-details"] a::attr(href)').getall()
        
        if not product_links:
            product_links = response.css('a[href*="/site/"]::attr(href)').getall()
        
        for link in product_links:
            if link and '/site/' in link and '.p?' in link:
    ...
    def parse_product_page(self, response):
        """
        Parse individual Best Buy product pages and extract detailed information
        """
        item = BestBuyProductItem()
        item['platform'] = 'bestbuy'
        ...
        item['currency'] = 'USD'  # Best Buy US uses USD
        ...
        item['totaltech_discount'] = self._extract_totaltech_discount(response)
    ...
    def _extract_brand(self, response) -> Optional[str]:
        ...
        if not brand:
            brand = response.css('span[data-automation-id="product-brand"]::text').get()
        if not brand:
    ...
    def _extract_description(self, response) -> Optional[str]:
        ...
        if not description:
            description = response.css('div[data-automation-id="product-description"]::text').get()
    ...
    def _extract_bullet_points(self, response) -> list:
        ...
        if not bullets:
            bullets = response.css('ul.product-highlights li::text').getall()
        if not bullets:
            bullets = response.css('ul[data-automation-id="product-highlights"] li::text').getall()
        return [bullet.strip() for bullet in bullets if bullet.strip()]

    def _extract_product_id(self, response) -> Optional[str]:
        """Extract Best Buy product ID (SKU)"""
        # Extract from URL
        url_match = re.search(r'/site/[^/]+/(\d+)\.p', response.url)
        if url_match:
        ...
        if product_id:
            match = re.search(r'/site/[^/]+/(\d+)\.p', product_id)
            if match:
                return match.group(1)
        
        # Try to extract SKU from page
        sku = response.css('span[data-testid="sku"]::text').get()
        if sku:
            return sku.strip()
    ...
    def _extract_current_price(self, response) -> Optional[float]:
        ...
        price_text = response.css('span[data-testid="price-current"]::text').get()
        if not price_text:
            price_text = response.css('span.price-current::text').get()
        if not price_text:
            price_text = response.css('span[data-automation-id="price-current"]::text').get()
    ...
    def _extract_original_price(self, response) -> Optional[float]:
        ...
        price_text = response.css('span[data-testid="price-original"]::text').get()
        if not price_text:
            price_text = response.css('span.price-original::text').get()
        if not price_text:
            price_text = response.css('span[data-automation-id="price-original"]::text').get()
    ...
    def _extract_availability(self, response) -> str:
        ...
        if not availability_text:
            availability_text = response.css('span.availability-status::text').get()
        if not availability_text:
            availability_text = response.css('span[data-automation-id="availability-status"]::text').get()
    ...
    def _extract_rating(self, response) -> Optional[float]:
        ...
        if not rating_text:
            rating_text = response.css('span.product-rating::text').get()
        if not rating_text:
            rating_text = response.css('span[data-automation-id="product-rating"]::text').get()
    ...
    def _extract_review_count(self, response) -> int:
        ...
        if not review_text:
            review_text = response.css('span[data-automation-id="review-count"]::text').get()
    ...
    def _extract_category(self, response) -> Optional[str]:
        ...
        category = response.css('nav[data-testid="breadcrumb"] a::text').getall()
    ...
    def _extract_subcategory(self, response) -> Optional[str]:
        ...
        breadcrumbs = response.css('nav[data-testid="breadcrumb"] a::text').getall()
    ...
    def _extract_images(self, response) -> list:
        ...
        # Primary image
        primary_img = response.css('img[data-testid="product-image"]::attr(src)').get()
        if primary_img:
            images.append(ImageItem(url=primary_img, type='primary'))
        
        # Gallery images
        gallery_imgs = response.css('div[data-testid="image-gallery"] img::attr(src)').getall()
        if not gallery_imgs:
            gallery_imgs = response.css('div.image-gallery img::attr(src)').getall()
    ...
    def _extract_specifications(self, response) -> list:
        ...
        spec_rows = response.css('table[data-testid="specifications"] tr').getall()
        if not spec_rows:
            spec_rows = response.css('table.specifications tr').getall()
    ...
    def _extract_variations(self, response) -> list:
        ...
        # Color variations
        color_options = response.css('div[data-testid="color-options"] button::attr(data-color)').getall()
        if not color_options:
            color_options = response.css('div.color-options button::attr(data-color)').getall()
        ...
        # Size variations
        size_options = response.css('div[data-testid="size-options"] button::text').getall()
        if not size_options:
            size_options = response.css('div.size-options button::text').getall()
    ...
    def _extract_store_pickup(self, response) -> bool:
        ...
        pickup_text = response.css('span[data-testid="store-pickup"]::text').get()
        if not pickup_text:
            pickup_text = response.css('span.store-pickup::text').get()
    ...
    def _extract_shipping_info(self, response) -> Optional[str]:
        ...
        shipping_text = response.css('span[data-testid="shipping-info"]::text').get()
        if not shipping_text:
            shipping_text = response.css('span.shipping-info::text').get()
        
        return shipping_text.strip() if shipping_text else None

    def _extract_totaltech_discount(self, response) -> Optional[float]:
        """Extract TotalTech discount percentage"""
        totaltech_text = response.css('span[data-testid="totaltech-discount"]::text').get()
        if not totaltech_text:
            totaltech_text = response.css('span.totaltech-discount::text').get()
        
        if totaltech_text:
            discount_match = re.search(r'(\d+)%', totaltech_text)
            if discount_match:
