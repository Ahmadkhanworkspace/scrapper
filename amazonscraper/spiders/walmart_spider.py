"""
Walmart Spider for Unified E-commerce Product Data Aggregator
"""
import scrapy
from scrapy import Request
from ..items import WalmartProductItem, ImageItem, SpecificationItem, VariationItem, ReviewItem
...
class WalmartSpider(BaseSpider):
    ...
    start_urls = [
        ...
        'https://www.walmart.com/browse/fashion',
        'https://www.walmart.com/browse/toys',
        'https://www.walmart.com/browse/automotive',
    ]
    
    def start_requests(self):
        """Start requests for Walmart categories"""
        for url in self.start_urls:
            yield self.make_request(url=url, callback=self.parse_category_page)

    def parse_category_page(self, response):
        """
        Parse Walmart category listing pages and extract product links
        """
        # Extract product links from search results
        product_links = response.css('div[data-testid="item-stack"] a[data-testid="product-title"]::attr(href)').getall()
        ...
        if not product_links:
            product_links = response.css('div[data-automation-id="product-title"] a::attr(href)').getall()
        
        for link in product_links:
            if link and '/ip/' in link:
    ...
    def parse_product_page(self, response):
        """
        Parse individual Walmart product pages and extract detailed information
        """
        ...
        item['currency'] = 'USD'  # Walmart US uses USD
        ...
        item['walmart_id'] = item['external_id']
    ...
    def _extract_brand(self, response) -> Optional[str]:
        ...
        if not brand:
            brand = response.css('span.prod-BrandName::text').get()
        if not brand:
    ...
    def _extract_description(self, response) -> Optional[str]:
        ...
        if not description:
            description = response.css('div.prod-ProductDescription::text').get()
        if not description:
    ...
    def _extract_bullet_points(self, response) -> list:
        ...
        bullets = response.css('div[data-testid="product-highlights"] li::text').getall()
        if not bullets:
            bullets = response.css('ul.prod-Highlights li::text').getall()
        if not bullets:
            bullets = response.css('div[data-automation-id="product-highlights"] li::text').getall()
    ...
    def _extract_product_id(self, response) -> Optional[str]:
        ...
        # Extract from URL
        url_match = re.search(r'/ip/([^/?]+)', response.url)
        if url_match:
        ...
        if product_id:
            match = re.search(r'/ip/([^/?]+)', product_id)
            if match:
    ...
    def _extract_current_price(self, response) -> Optional[float]:
        ...
        if not price_text:
            price_text = response.css('span.prod-Price-current::text').get()
        if not price_text:
    ...
    def _extract_original_price(self, response) -> Optional[float]:
        ...
        if not price_text:
            price_text = response.css('span.prod-Price-original::text').get()
        if not price_text:
    ...
    def _extract_availability(self, response) -> str:
        ...
        if not availability_text:
            availability_text = response.css('span.prod-AvailabilityStatus::text').get()
        if not availability_text:
    ...
    def _extract_rating(self, response) -> Optional[float]:
        ...
        rating_text = response.css('span[data-testid="rating"]::text').get()
        if not rating_text:
            rating_text = response.css('span.prod-Rating::text').get()
        if not rating_text:
            rating_text = response.css('span[data-automation-id="rating"]::text').get()
    ...
    def _extract_review_count(self, response) -> int:
        ...
        if not review_text:
            review_text = response.css('span.prod-ReviewCount::text').get()
        if not review_text:
    ...
    def _extract_images(self, response) -> list:
        ...
        if not gallery_imgs:
            gallery_imgs = response.css('div.prod-ImageGallery img::attr(src)').getall()
    ...
    def _extract_specifications(self, response) -> list:
        ...
        if not spec_rows:
            spec_rows = response.css('table.prod-Specifications tr').getall()
    ...
    def _extract_variations(self, response) -> list:
        ...
        if not color_options:
            color_options = response.css('div.prod-ColorOptions button::attr(data-color)').getall()
        ...
        if not size_options:
            size_options = response.css('div.prod-SizeOptions button::text').getall()
    ...
    def _extract_store_pickup(self, response) -> bool:
        ...
        if not pickup_text:
            pickup_text = response.css('span.prod-StorePickup::text').get()
    ...
    def _extract_shipping_info(self, response) -> Optional[str]:
        ...
        if not shipping_text:
            shipping_text = response.css('span.prod-ShippingInfo::text').get()
